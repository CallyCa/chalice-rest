...
```bash
curl -X POST -H "Content-Type: application/json" -d '{"username": "admin", "password": "some random password"}' http://127.0.0.1:5000/auth/login
```
...
You can use refresh token to retrieve a new access_token using the endpoint `/auth/refresh`
